...
module heat
  ...
  real(kind=sp), parameter :: DX = 0.01, DY = 0.01  ! Fixed grid spacing
...
contains
  ...
  subroutine initialize(field0)
    ...
    do j = 0, field0%ny+1
       do i = 0, field0%nx+1
          ds2 = (i - field0%nx / 2.0_sp + 1)**2 + &
               (j - field0%ny / 2.0_sp + 1)**2
          if (ds2 < radius2) then
             field0%data(i,j) = 5.0_sp
          else
             field0%data(i,j) = 65.0_sp
          end if
       end do
    end do

    ! Boundary conditions
    field0%data(:,0) = 20.0_sp
    field0%data(:,field0%ny+1) = 70.0_sp
    field0%data(0,:) = 85.0_sp
    field0%data(field0%nx+1,:) = 5.0_sp
  ...
  subroutine swap_fields(curr, prev)
    ...
    real(kind=sp), pointer, contiguous, dimension(:,:) :: tmp
  ...
  !   a (real(sp)): update equation constant
  !   dt (real(sp)): time step value
  subroutine evolve(curr, prev, a, dt)
    implicit none

    type(field), intent(inout) :: curr, prev
    real(kind=sp) :: a, dt
    real(kind=sp) :: dx2, dy2
    integer :: i, j, nx, ny

    ! Variables for memory access outside of a type
    real(kind=sp), pointer, contiguous, dimension(:,:) :: cdata, pdata
    ...
    do j = 1, ny
       do i = 1, nx
          cdata(i, j) = pdata(i, j) + a * dt * &
               & ((pdata(i-1, j) - 2.0_sp*pdata(i, j) + &
               &   pdata(i+1, j)) / dx2 + &
               &  (pdata(i, j-1) - 2.0_sp*pdata(i, j) + &
               &   pdata(i, j+1)) / dy2)
       end do
    end do
  ...
  subroutine output(curr, iter)
    ...
    integer :: nx, ny, stat
    real(kind=sp), pointer, contiguous :: cdata(:,:)
