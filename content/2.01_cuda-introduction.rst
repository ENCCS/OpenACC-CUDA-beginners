...
.. figure:: img/BlocksAndThreads.png
    ...
    A simple example of the division of threads (green squares) in blocks (yellow rectangles).
...
In CUDA this is done by adding so-called `kernel cofiguration <https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#programming-model>`_, ``<<<numBlocks, threadsPerBlock>>>``, to the function call:
